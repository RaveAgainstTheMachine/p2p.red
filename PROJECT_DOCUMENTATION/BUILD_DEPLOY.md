...
- Repository remotes (origin):
  - `ssh://git@<ip>:2222/frosty/<domain>.git`
  - `https://git.mcfrosty.com/frosty/<domain>.git`
...
```
# Example for Vite at http://<ip>:5180
CORS_ORIGIN=http://<ip>:5180 docker compose -f docker-compose.metadata.yml up -d --force-recreate metadata-api
```
...
- dev TURN (optional) -> `http://<ip>:5175`
...
- Internal admin dashboard UI shell on `dash.<domain>` (telemetry + ops toggles).
- OpenBao-backed admin auth + admin API endpoints for status/telemetry/toggles.
- Envoy routing and TLS bundle for `dash.<domain>`.
...
- Added contact emails in the footer (<email>, <email>).
...
```
LOCAL_P2P_HOST=<ip> ./automation/local-prod-certs.sh
```
...
```
BAO_ADDR=https://<bao-domain>:8200 BAO_TOKEN=*** ./automation/render-local-secrets.sh
```
...
```
LOCAL_P2P_HOST=<ip> LOCAL_HTTPS_PORT=8443 ./automation/build-local-prod-images.sh
```

**Start the local prod-parity stack**
```
LOCAL_P2P_HOST=<ip> LOCAL_HTTPS_PORT=8443 docker compose -f docker-compose.local-prod.yml up -d
```

**Preflight (download bridge + API + Envoy)**
```
INSECURE=1 LOCAL_P2P_HOST=<ip> LOCAL_HTTPS_PORT=8443 ./automation/local-prod-preflight.sh
```

**Access**
- App: `https://<ip>:8443`
- Envoy admin: `http://<ip>:9901`
...
# PeerJS prod routing is via <signal-domain> with /peerjs path.
# If PeerJS websocket fails in prod, confirm the build args include:
#   VITE_PEERJS_HOST=<signal-domain>
...
scp -i /home/frosty/.ssh/p2p_deploy \
  ...
  /opt/p2p-file-share/images/*.tar ubuntu@<ip>:/tmp/
```
...
```
curl -fsS https://<signal-domain>/peerjs/id
curl -sS -D - -o /dev/null --http1.1 \
  -H 'Connection: Upgrade' -H 'Upgrade: websocket' \
  -H 'Sec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==' \
  -H 'Sec-WebSocket-Version: 13' \
  -H 'Origin: https://<domain>' \
  'https://<signal-domain>/peerjs?key=peerjs&id=test&token=tok&version=1.5.5'
```
...
**Verification**
```
curl -sL https://<domain> | grep -oE '/assets/[^" ]+' | head -n 2
curl -sL https://<domain>/assets/<latest>.js | grep -i 'How it works\|FAQ'
curl -s https://<domain>/api/status | jq
```
...
- Envoy admin API must be reachable at `<ip>:9901` for weight shifts.
...
`<signal-domain>` has its own TLS cert and Envoy filter chain (PeerJS only).
Because the Envoy container owns :80/:443, issue certs with a short Envoy stop:
```
docker stop p2p-envoy
sudo certbot certonly --standalone -d <signal-domain>
...
**Reissue + Convert (example for <domain>):**
```
docker stop p2p-envoy
sudo certbot certonly --standalone --key-type rsa --rsa-key-size 2048 -d <domain> -d www.<domain>

sudo mkdir -p /var/snap/docker/common/p2p-envoy-certs
sudo openssl pkcs12 -export \
  -out /var/snap/docker/common/p2p-envoy-certs/<domain>.p12 \
  -inkey /etc/letsencrypt/live/<domain>/privkey.pem \
  -in /etc/letsencrypt/live/<domain>/fullchain.pem \
  -passout pass:p2pred

sudo chmod 755 /var/snap/docker/common/p2p-envoy-certs
sudo chmod 644 /var/snap/docker/common/p2p-envoy-certs/<domain>.p12
docker start p2p-envoy
```

Repeat for `<signal-domain>` and `plausible.<domain>` (matching filenames in `envoy.yaml`).

### Plausible Analytics (prod)
Plausible is self-hosted on the prod VPS and exposed via `plausible.<domain>`.

**First-party proxy (recommended):**
- Envoy on `<domain>` proxies:
...
docker run --rm --env-file /run/secrets/plausible.env \
  -e BASE_URL=https://plausible.<domain> \
...
- Envoy routes (`<domain>`, `<signal-domain>`) point to the correct upstreams.
...
scp -i /home/frosty/.ssh/p2p_deploy \
  -o "ProxyCommand=ssh -i /home/frosty/.ssh/p2p_dev_key -W %h:%p debian@<ip>" \
  /opt/p2p-file-share/images/metadata-api.tar ubuntu@<ip>:/tmp/metadata-api.tar
...
SECRETS_ENV_FILE=/run/secrets/metadata.env ./deploy-metadata-api.sh
```
Ports are bound to localhost only (`<ip>:3001/5432/6379`). Use SSH tunnel for remote access.
Health check:
```
curl http://localhost:3001/health
```
...
```
curl -sS -X POST "http://<ip>:9901/runtime_modify?traffic_split.app_blue=100&traffic_split.app_green=0"
```
...
## Public GitHub Sync (Sanitized Release)

We publish a sanitized copy of the codebase to the public GitHub repository:
- Public URL: `https://github.com/RaveAgainstTheMachine/<domain>`

To prevent leaking private Git history or any internal configuration secrets, you must use the automated publishing script which copies the files, redacts sensitive information, scans the codebase for secrets, and commits to the target repository.

### Initial Setup & Resetting the Public Repository
If the public GitHub repository contains old or unwanted Git history, it must be reset before pushing:
1. Create a clean local workspace for the public repository.
2. Initialize it and set the remote origin:
   ```bash
   mkdir -p /path/to/public-repo
   cd /path/to/public-repo
   git init
   git remote add origin <email>:RaveAgainstTheMachine/<domain>.git
   ```

### Publishing a New Release
To sanitize and publish the current state of the dev repository to the public repository:
1. Run the publishing script (providing the path to the target public repository):
   ```bash
   PUBLIC_REPO=/path/to/public-repo ./automation/publish-github.sh
   ```
2. The script will automatically:
   - Run `automation/public-sync.sh` to copy the files while excluding private directories (`.git`, `vacuum-test/`, keys/secrets).
   - Redact all domains (`<dev-domain>`, `<bao-domain>`, etc.), raw IP addresses, and private configurations.
   - Run security checks (scanning for raw secrets, IPs, keys, and emails).
   - Verify that the target repository's Git history shares no commits with the private dev repository.
   - Commit the clean changes to the target repository.
3. Push to GitHub from the public repository directory:
   ```bash
   cd /path/to/public-repo
   # Force push if resetting the remote repository history:
   git push -u origin master --force
   ```

### Safety Guardrails
- **Pre-Push Hook**: Direct pushes from the dev repository to GitHub are blocked by `.githooks/pre-push` to prevent accidental history leaks.
