...
## Summary
On 2026-05-11, the production site `<domain>` was reported down with a "No Healthy Upstream" error. Investigation revealed that the Envoy proxy had restarted and loaded a stale traffic split configuration from disk, pointing 100% of traffic to the `blue` environment which was not running.
...
3.  **Monitoring**: Deployed `health-sentinel.sh` to `bao` to monitor `<domain>` availability and notify Discord on failure.
