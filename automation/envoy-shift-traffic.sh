...
REPO_RUNTIME_DIR="/opt/p2p-file-share/envoy-runtime"
ENVOY_RUNTIME_DIR=${ENVOY_RUNTIME_DIR:-$REPO_RUNTIME_DIR}
...
# Update the target runtime directory (likely the Snap path in prod)
RUNTIME_DIR="${ENVOY_RUNTIME_DIR}/traffic_split"
if [ -d "$RUNTIME_DIR" ]; then
    echo "📝 Updating runtime path: $RUNTIME_DIR"
    echo "$BLUE_WEIGHT" | sudo tee "$RUNTIME_DIR/app_blue" >/dev/null
    echo "$GREEN_WEIGHT" | sudo tee "$RUNTIME_DIR/app_green" >/dev/null
fi

# Also update the repo path if it's different (to survive next deploy sync)
if [ "$ENVOY_RUNTIME_DIR" != "$REPO_RUNTIME_DIR" ] && [ -d "${REPO_RUNTIME_DIR}/traffic_split" ]; then
    echo "📝 Syncing to repo path: ${REPO_RUNTIME_DIR}/traffic_split"
    echo "$BLUE_WEIGHT" | sudo tee "${REPO_RUNTIME_DIR}/traffic_split/app_blue" >/dev/null
    echo "$GREEN_WEIGHT" | sudo tee "${REPO_RUNTIME_DIR}/traffic_split/app_green" >/dev/null
fi
