...
let setupAdminToken = null;
const configPath = process.env.CONFIG_PATH || '/app/data/config.json';
let siteConfig = {};

if (!fs.existsSync(configPath)) {
  isSetupMode = true;
  setupAdminToken = crypto.randomBytes(16).toString('hex');
  console.log('====================================================');
  console.log('                 SETUP MODE ACTIVE                  ');
  console.log('====================================================');
  console.log('  Admin Token for Setup:                            ');
  console.log(`  ${setupAdminToken} `);
  console.log('====================================================');
  console.log('  ACTION REQUIRED:                                  ');
  console.log('  Open your browser to http://localhost:8080        ');
  console.log('  to complete initial setup.                        ');
  console.log('                                                    ');
  console.log('  Web setup handles:                                ');
  console.log('  - Site Title / Branding                           ');
  console.log('  - Primary Color Theme                             ');
...
if (!ADMIN_JWT_SECRET || ADMIN_JWT_SECRET.length < 32) {
  console.error('FATAL: ADMIN_JWT_SECRET not set or too short (min 32 chars). Refusing to start.');
  process.exit(1);
}
...
let ADMIN_COOKIE_DOMAIN = process.env.ADMIN_COOKIE_DOMAIN || process.env.SITE_DOMAIN || '';
if (!ADMIN_COOKIE_DOMAIN && isProduction) {
  console.error('FATAL: ADMIN_COOKIE_DOMAIN or SITE_DOMAIN not set. Refusing to start.');
  process.exit(1);
} else if (!ADMIN_COOKIE_DOMAIN) {
...
const corsOrigin = (() => {
  if (corsOriginEnv === 'dev' || corsOriginEnv === '*') {
    ...
    return [`https://${siteDomain}`, `http://${siteDomain}`, 'http://localhost:5173', 'http://<ip>:5173', 'http://localhost:3000', 'http://<ip>:3000', 'http://localhost:8080', 'http://<ip>:8080'];
  }
...
function generateShortKey() {
  ...
  for (let i = 0; i < 16; i++) {
    const index = crypto.randomInt(0, 62);
    key += chars[index];
  }
...
function buildTurnCredentials() {
  ...
  // codeql[js/weak-cryptographic-algorithm] RFC 5389 mandates HMAC-SHA1 for TURN long-term credentials
...
// TURN credential rate limiter — tighter than the global API limiter
const turnCredLimiter = rateLimit({
...
const requireInternalAccess = (req, res, next) => {
  const ip = req.ip || '';
  if (ip === '<ip>' || ip === '::1' || ip === '::ffff:<ip>') return next();
...
app.get('/api/status', async (req, res) => {
  try {
    ...
    res.json({ ...payload, setupMode: isSetupMode, siteConfig });
  } catch (error) {
...
app.post('/api/setup', async (req, res) => {
  if (!isSetupMode) return res.status(403).json({ error: 'Setup already completed' });
  const token = req.headers['x-setup-token'];
  if (!token || token !== setupAdminToken) {
    return res.status(401).json({ error: 'Invalid admin token' });
  }
  
  const { siteTitle, primaryColor, features } = req.body;
  try {
    const newConfig = { siteTitle, primaryColor, features };
      ...
      [
        ...
        metadata && typeof metadata === 'object' ? (() => {
          try {
            ...
            return str.length > 2048 ? { error: 'metadata payload exceeded limits' } : metadata;
          } catch {
            return {};
          }
        })() : {}
...
app.get('/share/:key', limiter, async (req, res) => {
  try {
    ...
    if (!key || key.length !== 16 || !/^[a-zA-Z0-9]{16}$/.test(key)) {
      return res.status(400).send(`
        ...
        <html>
          <head>
            <meta http-equiv="refresh" content="0; url=https://p2p.red/#${escapeHtml(key)}">
          </head>
          <body>
...
<html lang="en">
  ...
  <html lang="en">
    <head>
      <meta http-equiv="refresh" content="0; url=https://p2p.red/#${escapeHtml(req.params.key)}" />
    </head>
    <body>
      <p>Redirecting to P2P File Share...</p>
    </body>
...
app.get('/api/stats', requireInternalAccess, async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT 
