...
import { ResumeSession, RESUME_SESSION_KEY } from '../utils/resumeSessions';

export interface AdaptiveTransferProgress {
...
const createCryptoWorkerClient = (): CryptoWorkerClient | null => {
  ...
  try {
    ...
    debugWarn('⚠️ Crypto worker unavailable', error);
    return null;
  }
  let requestId = 0;
...
export const createTransferEngine = () => {
  ...
  let transferProgressState: AdaptiveTransferProgress = {
    bytesTransferred: 0,
    totalBytes: 0,
    percentage: 0,
    speed: 0,
  ...
  const setTransferProgressState = (next: AdaptiveTransferProgress | ((prev: AdaptiveTransferProgress) => AdaptiveTransferProgress)) => {
    transferProgressState = typeof next === 'function' ? next(transferProgressState) : next;
    emitter.emit('progress', transferProgressState);
  };
  const transferProgress = { current: transferProgressState } as { current: AdaptiveTransferProgress };

  const isTransferring = { current: false } as { current: boolean };
  const startTime = { current: 0 } as { current: number };

  const setIsTransferring = (value: boolean) => {
    isTransferring.current = value;
  };

  const setTransferProgress = (next: AdaptiveTransferProgress) => {
  ...
  const performEcdhHandshake = async (
    conn: DataConnection,
    ...
    return new Promise((resolve, reject) => {
      ...
      const handler = async (rawData: unknown) => {
  ...
  const createParallelChannels = async (
    baseConn: DataConnection,
    streamCount: number,
    isSender: boolean
  ): Promise<RTCDataChannel[]> => {
    debugLog(`🎯 Creating ${streamCount} parallel channels (${isSender ? 'SENDER' : 'RECEIVER'})`);
    ...
    try {
      ...
      debugLog(`✅ Orchestrator: ${channels.length}/${streamCount} channels ready`);
      return channels;
    } catch (error) {
      debugError('❌ Orchestrator failed:', error);
      return [];
    }
  };

  // Send any stream (File or ReadableStream) with shard-based parallel transfer
  const sendFileMultiStream = async (
    conn: DataConnection,
    input: File | ReadableStream,
    fileName?: string,
    totalSize?: number
  ): Promise<void> => {
    debugLog('🚀 sendFileMultiStream STARTED (SHARD-BASED)');
    return new Promise<void>(async (resolve, reject) => {
      ...
      try {
        ...
        if (encryptionState?.enabled && cryptoWorker) {
          try {
            ...
            debugWarn('⚠️ Crypto worker init failed, falling back to main thread', error);
            cryptoWorker.terminate();
          }
        }
      } catch (error) {
        debugError('❌ ECDH handshake failed (sender):', error);
      ...
      debugLog(`📊 Shard size: ${(SHARD_SIZE/1024/1024).toFixed(0)}MB (based on estimated ${(estimatedSpeed/1024/1024).toFixed(0)}MB/s)`);
      
      // Create shards
      const shards = createShards(fileSize, SHARD_SIZE);
      debugLog(`🧩 Created ${shards.length} shards for ${(fileSize/1024/1024/1024).toFixed(2)}GB file`);
      ...
      debugLog(`🔧 Using ${numChannels} adaptive channels for ${shards.length} shards`);
      ...
      const receiverBackpressureHandler = (rawData: unknown) => {
      const data = rawData as TransferMessage;
      ...
      conn.on('data', receiverBackpressureHandler as (data: unknown) => void);

      try {
        // Send metadata first
        debugLog('📤 Sending shard metadata...');
        const metadata = {
        ...
        await new Promise<void>((resolveReady) => {
          const readyHandler = (rawData: unknown) => {
      const data = rawData as TransferMessage;
            ...
            if (data?.type === 'receiver_ready') {
              ...
              conn.off('data', readyHandler as (data: unknown) => void);
              resolveReady();
            }
          };
          conn.on('data', readyHandler as (data: unknown) => void);
        });

        const waitForResumeState = () => new Promise<void>((resolveResume) => {
          const resumeHandler = (rawData: unknown) => {
      const data = rawData as TransferMessage;
            ...
            if (data?.type === 'resume_state' && data.transferId === transferId) {
              ...
              conn.off('data', resumeHandler as (data: unknown) => void);
              resolveResume();
            }
          };
          conn.on('data', resumeHandler as (data: unknown) => void);
          setTimeout(() => {
            conn.off('data', resumeHandler as (data: unknown) => void);
            resolveResume();
          }, 2000);
        ...
        let deviceMax = 64;
        const cores = receiverProfile?.hardwareConcurrency ?? navigator.hardwareConcurrency;
        ...
        if (adjustedChannels !== numChannels) {
          ...
          debugLog('📡 Sent stream_count_update', {
        ...
        debugLog('🔧 Creating DataChannels...');
        ...
        const waitForChannelBufferRoom = async (channel: RTCDataChannel) => {
          while (channel.bufferedAmount > CHANNEL_BUFFER_HIGH_WATERMARK) {
            ...
            if (localStorage.p2p_debug_transfer === '1') {
              debugLog(`🕰️ Waited ${perfNow() - waitStart}ms for channel ${channel.label} to drain`);
            }
          }
        };
        ...
        if (typeof sctpMax === 'number' && sctpMax > 0) {
          debugLog(`📦 Chunk size: ${(CHUNK_SIZE / 1024).toFixed(0)}KB (SCTP max ${(sctpMax / 1024).toFixed(0)}KB, overhead ${encryptionOverhead} bytes)`);
        }
        ...
        const messageHandler = async (rawData: unknown) => {
      const data = rawData as TransferMessage;
          ...
          if (data.type === 'shard_confirmed') {
            ...
            if (shardCache.has(shardId)) {
              ...
              notifyCacheWaiters();
              debugLog(`✅ Shard ${shardId} confirmed, cleared from cache (${shardCache.size} shards cached)`);
            }
            ...
            for (const shardId of data.shardIds) {
              ...
              if (isFile) {
                ...
                const cached = shardCache.get(shardId);
                if (!cached) {
                  debugError(`❌ Shard ${shardId} not in cache, cannot resend metadata`);
                  continue;
                }
                shardData = cached;
              }
            ...
            debugLog(`📡 Retransmit request for ${data.shardIds.length} shards`);
            ...
            for (const shardId of data.shardIds) {
              ...
              if (channel.readyState !== 'open') {
                debugError(`❌ Channel ${shard.channelId} closed, cannot retransmit shard ${shardId}`);
                continue;
              }

              debugLog(`🔄 Retransmitting shard ${shardId} (${(shard.size/1024/1024).toFixed(1)}MB)`);
              ...
              if (isFile) {
                ...
                const cached = shardCache.get(shardId);
                if (!cached) {
                  debugError(`❌ Shard ${shardId} not in cache, cannot retransmit`);
                  continue;
                }
                shardData = cached;
              }
        ...
        conn.on('data', messageHandler as (data: unknown) => void);
        ...
        if (isFile) {
          ...
          debugLog(`📊 Shard distribution: ${channels.length} channels handling ${shards.length} shards`);
            ...
            for (const shard of assignedShards) {
              ...
              while (sentBytes < shardData.length) {
                ...
                if (currentPercentage >= lastLoggedPercentage + 10) {
                  debugLog(`📤 ${currentPercentage}% sent (${(totalBytesTransferred/1024/1024/1024).toFixed(2)}GB / ${(fileSize/1024/1024/1024).toFixed(2)}GB) @ ${(speed/1024/1024).toFixed(1)}MB/s`);
          ...
          debugLog('📦 ReadableStream mode - sequential shard processing');
          ...
          if (currentShardFilled > 0 && currentShardIndex < shards.length) {
            const shard = shards[currentShardIndex];
            if (!shard) {
              debugLog('✅ All shards sent, stream complete');
        ...
        const waitForReceiverDone = () => new Promise<void>((resolveDone) => {
          const doneHandler = (rawData: unknown) => {
      const data = rawData as TransferMessage;
            ...
            if (data?.type === 'receiver_done') {
              receiverDone = true;
              clearInterval(resendInterval);
              conn.off('data', doneHandler as (data: unknown) => void);
              resolveDone();
            }
          };
          conn.on('data', doneHandler as (data: unknown) => void);
        ...
        conn.off('data', messageHandler as (data: unknown) => void);

        conn.off('data', receiverBackpressureHandler as (data: unknown) => void);
        ...
        debugError('❌ Shard transfer error:', error);
        conn.off('data', receiverBackpressureHandler as (data: unknown) => void);
        clearInterval(perfInterval);
        cryptoWorker?.terminate();
        setIsTransferring(false);
  ...
  const prepareDownloadBridge = async (fileName: string, fileSize: number) => {
    ...
    return new Promise<string | null>((resolve) => {
      ...
      const timeout = window.setTimeout(() => {
        cleanup();
        debugWarn('⚠️ Download bridge timeout');
        resolve(null);
      }, 8000);
      ...
      channel.port1.onmessage = (event) => {
        if (event.data?.debug === 'download_ready') {
          debugLog('✅ Download bridge ready');
        }
  ...
  const receiveFileMultiStream = async (
    conn: DataConnection,
    options?: ReceiveOptions
  ): Promise<void> => {
    debugLog('📥 receiveFileMultiStream STARTED (SHARD-BASED)');
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const handshakePromise = performEcdhHandshake(conn, 'receiver')
        ...
        .catch((error) => {
          debugError('❌ ECDH handshake failed (receiver):', error);
          setIsTransferring(false);
          reject(error instanceof Error ? error : new Error('ECDH handshake failed'));
        });
      ...
      const verifyAndConfirmShard = async (shardId: number, shardState: { received: number; total: number; complete: boolean; expectedCRC: string; currentCRC: number; attemptId: number; offset: number; }) => {
        ...
        debugError(`❌ Shard ${shardId} CRC32 mismatch! Expected: ${shardState.expectedCRC}, Got: ${actualCRC}`);
      ...
      const pumpWrites = () => {
        ...
        while (inFlightWrites < MAX_INFLIGHT_WRITES && writeQueue.length > 0) {
          ...
          inFlightWrites++;
          (writableStream as FileSystemWritableFileStream).write({
            type: 'write',
            position: item.position,
            data: item.data as ArrayBufferView<ArrayBuffer>
          }).then(() => {
            inFlightWrites--;
            ...
            queuedOrInFlightBytes -= item.size;
            maybeSendBackpressure();
            debugError('❌ Chunk write failed:', error);
            pumpWrites();
          });
        }
      };
      ...
      const finalizeIfReady = async () => {
        ...
        if (!completeSignalReceived) {
          if (expectedBytes > 0 && bytesReceived === expectedBytes) {
            debugWarn('⚠️ Missing complete signal, finalizing based on shard completion', {
              bytesReceived,
              expectedBytes
            });
        ...
        if (expectedTotalBytes !== null && bytesReceived !== expectedTotalBytes) {
          debugError('❌ Total bytes mismatch', {
        ...
        if (expectedTotalCrc32) {
          ...
          if (!useFileSystemAPI && !useBlobDownload && transferId && shardCount > 0) {
            ...
            for (let i = 0; i < shardCount; i++) {
              ...
              if (!shardData) {
                debugError('❌ Missing shard data for total CRC32 recompute', { shardId: i });
          ...
          if (actualTotalCrc32 !== expectedTotalCrc32) {
            debugError('❌ Total CRC32 mismatch', {
        ...
        if (useFileSystemAPI && writableStream) {
          ...
          await (writableStream as FileSystemWritableFileStream).close();
        ...
        if (!useFileSystemAPI) {
          if (downloadKey && !useBlobDownload && downloadBridgeCompletePromise) {
            ...
            if (!completionObserved) {
              debugWarn('⚠️ Download bridge completion timeout', { downloadKey });
            }
          }
          conn.send({ type: 'receiver_done', timestamp: Date.now() });
          clearInterval(perfInterval);
        ...
        if (useBlobDownload) {
          ...
          for (let i = 0; i < shardCount; i++) {
            ...
            const state = shardStates.get(i);
            if (!buf || !state) {
              debugError('❌ Missing shard buffer during blob finalize', { shardId: i });
              setIsTransferring(false);
        ...
        debugError('❌ No save method available after transfer');
        clearInterval(perfInterval);
        clearInterval(stallInterval);
        cryptoWorker?.terminate();
        setIsTransferring(false);
      ...
      let writableStream: unknown = null;
      ...
      const setupDataChannelHandlers = () => {
        ...
        if (!pc) {
          debugError('❌ PeerConnection not available yet');
          return;
        }
        
        debugLog('✅ Setting up DataChannel handlers on receiver');
        
        pc.ondatachannel = (event: RTCDataChannelEvent) => {
          debugLog(`📡 Received DataChannel: ${event.channel.label}`);
            ...
            const next = queued
              ...
              .then(async () => {
                ...
                if (encryptionEnabled) {
                  if (!encryptionState) {
                    debugError('❌ Encrypted chunk received before ECDH handshake');
                    return;
                  }
                  const ivStart = 8;
                  ...
                  try {
                    ...
                    debugError('❌ Failed to decrypt chunk', error);
                    return;
                  }
                }
                ...
                if (!shardStates.has(shardId)) {
                  debugError(`❌ Received data for unknown shard ${shardId}`);
                  return;
                }
                ...
                if (shardState.received >= shardState.total) {
                  if (!shardState.expectedCRC) {
                    debugWarn('⚠️ Shard completed without CRC metadata, finalizing based on byte count', {
                      shardId,
                  ...
                  debugLog(`🔍 Shard ${shardId} complete: ${shardState.received}/${shardState.total} bytes, expectedCRC: ${shardState.expectedCRC}`);
                  ...
                  if (shardVerified && !useFileSystemAPI && transferId && !storedShardIds.has(shardId)) {
                    const shardBuffer = shardBuffers.get(shardId);
                    if (!shardBuffer) {
                      debugWarn('⚠️ Shard buffer missing on completion', { shardId });
                    } else {
                      ...
                      try {
                        ...
                        debugError('❌ Failed to persist shard to IndexedDB', error);
                      }
                    }
                  }
                }
                ...
                // Log every 10% milestone
                if (currentPercentage >= lastLoggedPercentage + 10) {
                  debugLog(`📥 ${currentPercentage}% received (${(bytesReceived/1024/1024/1024).toFixed(2)}GB / ${(fileSize/1024/1024/1024).toFixed(2)}GB) @ ${(speed/1024/1024).toFixed(1)}MB/s`);
                  lastLoggedPercentage = currentPercentage;
                }
      ...
      const handleMainMessage = async (rawData: unknown) => {
      const data = rawData as TransferMessage;
        ...
        if (data.type === 'multi_stream_start') {
          ...
          try {
            ...
            debugError('❌ ECDH handshake failed before metadata:', error);
            return;
          }
          ...
          if (!transferId) {
            debugError('❌ Missing transferId for encrypted transfer');
            setIsTransferring(false);
          ...
          if (!data.encryption?.enabled) {
            debugError('❌ Encryption metadata missing from sender');
            setIsTransferring(false);
          ...
          if (data.encryption?.enabled && !encryptionState) {
            debugError('❌ Encryption metadata received without ECDH key');
            setIsTransferring(false);
          ...
          debugLog(`📊 Expecting ${shardCount} shards of ${(shardSize / 1024 / 1024).toFixed(0)}MB each`);
          ...
          if (options?.fileHandle) {
            try {
              ...
              writableStream = await (fileHandle as FileSystemFileHandle).createWritable({ keepExistingData: false });
              useFileSystemAPI = true;
              debugLog('✅ File System Access API active', { name: (fileHandle as FileSystemFileHandle)?.name ?? 'unnamed' });
            } catch (err) {
              debugError('❌ File System Access API failed', err);
          ...
          if (!useFileSystemAPI) {
            ...
            if (!useBlobDownload && !downloadKey && requireSave) {
              debugError('❌ No save method available (File System Access + Download bridge unavailable)');
              setIsTransferring(false);
              reject(new Error('No save method available'));
              return;
            }
          ...
          await new Promise<void>((resolve) => {
            const checkConnection = () => {
              ...
              if (pc && pc.connectionState !== 'closed') {
                debugLog(`✅ PeerConnection ready (state: ${pc.connectionState})`);
                ...
                debugLog('⏳ Waiting for peerConnection...');
          ...
          debugLog('📡 Sending receiver_ready', {
            transferId,
          ...
          debugLog('✅ Transfer complete signal (sender)');
          ...
          const requestMissingAfterSettle = (retriesLeft: number) => {
            ...
            if (missingShards.length > 0) {
              debugError(`❌ Missing ${missingShards.length} shards:`, missingShards.slice(0, 10));
              conn.send({
                type: 'retransmit_shards',
      ...
      conn.on('data', handleMainMessage as (data: unknown) => void);
    });
  };

  const transferFileAdaptive = async (
    conn: DataConnection,
