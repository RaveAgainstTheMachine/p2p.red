...
const fetchWithChallenge = async (url: string, options: RequestInit): Promise<Response> => {
  ...
  if (isLikelyChallenge) {
    debugWarn('🛡️ Bot challenge detected for:', url, 'Header:', isAnubisHeader, 'HTML:', contentType.includes('text/html'));
    ...
    for (let attempt = 0; attempt < 15; attempt += 1) {
      debugLog(`⏳ Waiting for challenge solution (attempt ${attempt + 1})...`);
      await delay(2000); 
...
export async function createShortLink(metadata: TransferMetadata, pin?: string): Promise<string> {
  try {
    ...
    debugLog('📡 createShortLink payload:', { 
      ...payload, 
    ...
    if (!response.ok) {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        throw new Error((error.error as string) || `HTTP ${response.status}: ${response.statusText}`);
      } else {
        const text = await response.text();
        debugError('📡 Non-JSON error response from metadata API:', text);
    ...
    if (contentType && contentType.includes('application/json')) {
      ...
      debugLog(`✅ Short link created: ${data.key}, expires: ${data.expiresAt}`);
      return data.key;
    } else {
      const text = await response.text();
      debugError('📡 Unexpected non-JSON success response:', text);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        const text = await response.text();
        debugError('📡 Non-JSON error response from getMetadata:', text);
      ...
      if (response.status === 401 && error.requiresPin) {
        ...
        (err as Error & { requiresPin: boolean }).requiresPin = true;
        (err as Error & { pinType: string | null }).pinType = (error.pinType as string) || null;
        throw err;
      }
      if (response.status === 403) {
        const err = new Error((error.message as string) || 'Invalid PIN');
        (err as Error & { remainingAttempts: number }).remainingAttempts = error.remainingAttempts as number;
        throw err;
      }
      if (response.status === 429) {
        throw new Error((error.message as string) || 'Too many PIN attempts. Please try again later.');
      }
      throw new Error((error.error as string) || (error.message as string) || `HTTP ${response.status}: ${response.statusText}`);
    }

    const contentType = response.headers.get('content-type');
    if (contentType && contentType.includes('application/json')) {
      const metadata: TransferMetadata = await response.json();
      debugLog(`📥 Retrieved metadata for key: ${key}`, metadata);
      return metadata;
    } else {
      const text = await response.text();
      debugError('📡 Unexpected non-JSON success response from getMetadata:', text);
      throw new Error('Server returned invalid response format (not JSON)');
    }
  } catch (error) {
    debugError('Failed to retrieve metadata:', error);
